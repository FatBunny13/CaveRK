...
def render_all(con, panel, entities, player, game_map, fov_map, fov_recompute, message_log, screen_width, screen_height,
            ...
            for x in range(game_map.width):
                ...
                if visible and game_map.red_cave_level > 0:
                    ...
                    if wall:
                        ...
                        libtcod.console_set_char_background(con, x, y, colors['moth_ground'], libtcod.BKGND_SET)

                    game_map.tiles[x][y].explored = True
                elif visible and game_map.wyld == True:
                    if wall:
                        libtcod.console_set_char_background(con, x, y, colors['forest_wall'], libtcod.BKGND_SET)
                    else:
                        libtcod.console_set_char_background(con, x, y, colors['forest_ground'], libtcod.BKGND_SET)
    ...
    libtcod.console_print_ex(panel, 1, 5, libtcod.BKGND_NONE, libtcod.LEFT,
                             'Poisoned: {0}'.format(player.fighter.poisoned))
    if game_map.red_cave_level > 0:
        libtcod.console_print_ex(panel, 1, 5, libtcod.BKGND_NONE, libtcod.LEFT,
